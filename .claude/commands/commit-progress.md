...
Commit uncommitted study work across **three repos**, each with its own scope, branch convention, and commit-message format. This supersedes the old `commit-daily-leetcode` command and fully covers it (the daily-LeetCode case is Target A below).
...
- Discover pending work per repo with:
  - `git status --porcelain src/leetcode/daily/`
  - `git -C courses/dsa-mentorship status --porcelain`
  - `git -C courses/fse13-faang status --porcelain`
- **Never** run `git add -A`. Stage only the exact files of the unit being committed. Leave everything out of scope untouched (other submodules, `src/leetcode/problems/**`, `build/`, slides, READMEs, etc.).
...
`<technique>` everywhere = the **core idea**, max **5-6 words** (Targets A/B) / **5-10 words** (Target C), concise.
